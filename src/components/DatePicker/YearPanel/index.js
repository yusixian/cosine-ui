...
const YearHeader = ({ curYear, changeYear }) => {
  ...
  return (
    <nav className="picker-panel-header">
      <a href="#!" onClick={() => changeYear(startYear - 10)}>
        &#8249;&#8249;
      </a>
      <div>
        <a href="#!">
...
const Years = ({ curYear, onClick }) => {
  ...
  years.push(<Year key={startYear - 1} onClick={(y) => onClick(y)} curYear={curYear} year={startYear - 1} />)
  for (let i = 0; i < 10; ++i) {
    years.push(<Year key={startYear + i} onClick={(y) => onClick(y)} curYear={curYear} year={startYear + i} />)
  }
  years.push(<Year key={endYear + 1} onClick={(y) => onClick(y)} curYear={curYear} year={endYear + 1} />)
  return <nav className="picker-panel-years">{years.concat()}</nav>
}
  ...
  render() {
    ...
    return (
      <div className="picker-panel" data-active={active}>
        <YearHeader curYear={curYear} changeYear={(y) => this.changeYear(y)} />
